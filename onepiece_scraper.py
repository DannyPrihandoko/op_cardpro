...
SETS: dict[str, dict] = {
    ...
    "OP-15": {
        ...
        "product_page": f"{BASE_URL}/cardlist/?series=556115",
    },
    # ── EXTRA BOOSTER (EB) ─────────────────────────────────────────────
    "EB-01": {
        "series_id": "556201",
...
def card_image_url(card_id: str) -> str:
    ...
    Pola: /images/cardlist/card/{CARD_ID}.png
    """
    return f"{BASE_URL}/images/cardlist/card/{card_id}.png"
...
def parse_cards_from_html(html: str) -> list[dict]:
    ...
    dls = soup.find_all("dl", class_="modalCol")

    cards, seen = [], set()
    for dl in dls:
        card = _parse_section(dl)
...
def _parse_section(dl) -> dict | None:
    from bs4 import BeautifulSoup, Tag
    if isinstance(dl, str):
        soup = BeautifulSoup(dl, "html.parser")
        dl = soup.find("dl", class_="modalCol") or soup.find("dl")
        if not dl:
            return None
    ...
    # infoCol spans
    info_div = dl.find("div", class_="infoCol")
    rarity = None
    card_type = None
    if info_div:
        ...
        if len(spans) >= 3:
            ...
            card_type = spans[2].get_text(strip=True).title()

    # Name
    name_div = dl.find("div", class_="cardName")
    name = name_div.get_text(strip=True) if name_div else ""

    # Image URL
    img_tag = dl.find("img", class_="lazy")
    card_image = None
    if img_tag:
        ...
        if data_src:
            ...
            if not clean_src.startswith("http"):
                card_image = f"{BASE_URL}{clean_src}"
            else:
                card_image = clean_src
    if not card_image:
    ...
    card = dict(
        ...
        trigger=None, card_set=None, card_image_url=card_image,
    )

    back = dl.find("div", class_="backCol")
    if back:
        # Cost / Life
        cost_div = back.find("div", class_="cost")
        if cost_div:
            h3 = cost_div.find("h3")
            h3_text = h3.get_text(strip=True).lower() if h3 else ""
            val = cost_div.get_text(strip=True).replace(h3.get_text(strip=True) if h3 else "", "").strip()
            if val != "-":
                if "life" in h3_text:
                    card["life"] = val
                else:
        ...
        # Attribute
        attr_div = back.find("div", class_="attribute")
        if attr_div:
            i_tag = attr_div.find("i")
            if i_tag:
                val = i_tag.get_text(strip=True)
                if val != "-":
                    card["attribute"] = val

        # Power
        power_div = back.find("div", class_="power")
        if power_div:
            h3 = power_div.find("h3")
            val = power_div.get_text(strip=True).replace(h3.get_text(strip=True) if h3 else "", "").strip()
            if val != "-":
                card["power"] = val

        # Counter
        counter_div = back.find("div", class_="counter")
        if counter_div:
            h3 = counter_div.find("h3")
            val = counter_div.get_text(strip=True).replace(h3.get_text(strip=True) if h3 else "", "").strip()
            if val != "-":
                card["counter"] = val

        # Color
        color_div = back.find("div", class_="color")
        if color_div:
            h3 = color_div.find("h3")
            val = color_div.get_text(strip=True).replace(h3.get_text(strip=True) if h3 else "", "").strip()
            if val != "-":
                card["color"] = val

        # Block Icon
        block_div = back.find("div", class_="block")
        if block_div:
            h3 = block_div.find("h3")
            val = block_div.get_text(strip=True).replace(h3.get_text(strip=True) if h3 else "", "").strip()
            if val != "-":
                card["block_icon"] = val

        # Type (feature class)
        feat_div = back.find("div", class_="feature")
        if feat_div:
            h3 = feat_div.find("h3")
            val = feat_div.get_text(strip=True).replace(h3.get_text(strip=True) if h3 else "", "").strip()
            if val != "-":
                card["type"] = val

        # Effect (text class)
        text_div = back.find("div", class_="text")
        if text_div:
            h3 = text_div.find("h3")
            for br in text_div.find_all("br"):
                br.replace_with("\n")
            val = text_div.get_text().replace(h3.get_text() if h3 else "", "").strip()
            if val != "-":
                card["effect"] = val

        # Trigger
        trig_div = back.find("div", class_="trigger")
        if trig_div:
            h3 = trig_div.find("h3")
            for br in trig_div.find_all("br"):
                br.replace_with("\n")
            val = trig_div.get_text().replace(h3.get_text() if h3 else "", "").strip()
            if val != "-":
                card["trigger"] = val

        # Card Set (getInfo class)
        set_div = back.find("div", class_="getInfo")
        if set_div:
            h3 = set_div.find("h3")
            val = set_div.get_text(strip=True).replace(h3.get_text(strip=True) if h3 else "", "").strip()
            if val != "-":
                card["card_set"] = val
...
def install_playwright_deps() -> None:
    ...
    Wajib dijalankan sekali di Linux / Google Colab:
        python onepiece_scraper.py --install-deps
    """
    import sys
    import subprocess
    steps = [
        ([sys.executable, "-m", "pip", "install", "-q", "playwright", "beautifulsoup4"],
         "Install Python packages"),
        ([sys.executable, "-m", "playwright", "install", "chromium"],
         "Download Chromium browser"),
        ([sys.executable, "-m", "playwright", "install-deps", "chromium"],
         "Install system libraries (libatk, dll.)"),
    ]
    for cmd, desc in steps:
        ...
        for line in out.splitlines()[-5:]:
            print(f"   {line}")
        print(f"   {'✅' if r.returncode == 0 else '⚠️ '} exit={r.returncode}")
    print("\n✅  Selesai! Coba jalankan scraper kembali.\n")


def _libatk_hint(exc: Exception) -> None:
    """Deteksi error libatk / shared library dan tampilkan solusi."""
    msg = str(exc)
    if any(k in msg for k in ("libatk", "shared libraries", "cannot open shared object",
                               "BrowserType.launch", "has been closed")):
        print("\n" + "=" * 62)
        print("ERROR: Chromium gagal – library sistem tidak ditemukan.")
        print("(Umum terjadi di Google Colab & server Linux baru)\n")
        print("Solusi – jalankan cell ini DULU:\n")
        print("    !python onepiece_scraper.py --install-deps\n")
        print("Atau manual:")
        print("    !pip install playwright beautifulsoup4 -q")
...
def scrape_set(series_id: str, delay: float = 0) -> list[dict]:
    """Scrape satu set berdasarkan series_id menggunakan Playwright."""
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("Playwright belum terinstall.")
        print("Jalankan: python onepiece_scraper.py --install-deps")
        sys.exit(1)
...
def main() -> None:
    # Ensure UTF-8 output encoding for Windows terminal to avoid UnicodeEncodeError
    ...
    parser.add_argument("--install-deps", action="store_true",
        help="Install semua dependency Playwright (wajib di Colab/Linux)")
    ...
    # Load existing sets_index.json to preserve already scraped sets
    results: dict[str, dict] = {}
    index_path = OUTPUT_DIR / "sets_index.json"
    if index_path.exists():
        try:
            with open(index_path, "r", encoding="utf-8") as f:
                old_index = json.load(f)
                for code, entry in old_index.items():
                    if entry.get("scraped"):
                        results[code] = {
                            "total_cards": entry.get("total_cards", 0),
                            "scraped": True
                        }
        except Exception:
            pass
